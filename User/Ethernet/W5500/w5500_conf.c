...
**
* @brief
*配置MCU，移植w5500程序需要修改的文件，配置w5500的MAC和IP地址 *
******************************************************************************
* @company  		深圳炜世科技有限公司 *
...
/*定义默认IP信息*/
uint8 local_ip[4] = {192, 168, 32, 249};    /*定义W5500默认IP地址*/
uint8 subnet[4] = {255, 255, 255, 0};       /*定义W5500默认子网掩码*/
uint8 gateway[4] = {192, 168, 32, 1};       /*定义W5500默认网关*/
...
void set_w5500_ip(void) {
  ...
  if (ip_from == IP_FROM_EEPROM) {
    ...
    if (*(EEPROM_MSG.mac) == 0x00 && *(EEPROM_MSG.mac + 1) == 0x08 &&
        *(EEPROM_MSG.mac + 2) == 0xdc) {
...
void gpio_for_w5500_config(void) {
  ...
  GPIO_InitStructure.GPIO_Mode =
      GPIO_Mode_Out_PP; /*设置引脚模式为通用推挽输出*/
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; /*设置引脚速率为50MHz */
  GPIO_InitStructure.GPIO_Mode =
      GPIO_Mode_IPU; /*设置引脚模式为通用推挽模拟上拉输入*/
  GPIO_Init(GPIOB, &GPIO_InitStructure); /*调用库函数，初始化GPIO*/
}
 ...
 *@return	无
 */
void iinchip_csoff(void) { wiz_cs(LOW); }
 ...
 *@return	无
 */
void iinchip_cson(void) { wiz_cs(HIGH); }
 ...
 *@return	无
 */
uint8 IINCHIP_SpiSendData(uint8 dat) { return (SPI_FLASH_SendByte(dat)); }
 ...
 *@return	无
 */
void dhcp_timer_init(void) { timer2_init(); }
 ...
 *@return	无
 */
void ntp_timer_init(void) { timer2_init(); }
