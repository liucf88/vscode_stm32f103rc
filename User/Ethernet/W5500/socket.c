...
*@brief   This Socket function initialize the channel in perticular mode,
										and set the port and wait for W5200 done
it.
...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
	...
	if (((protocol & 0x0F) == Sn_MR_TCP) || ((protocol & 0x0F) == Sn_MR_UDP) ||
		...
		if (port != 0) {
			...
			local_port++;  // if don't set the source port, set local_port
						   // number.
	...
	if (((addr[0] == 0xFF) && (addr[1] == 0xFF) && (addr[2] == 0xFF) &&
		...
		while (IINCHIP_READ(Sn_SR(s)) != SOCK_SYNSENT) {
			...
			if (getSn_IR(s) & Sn_IR_TIMEOUT) {
				IINCHIP_WRITE(Sn_IR(s),
							  (Sn_IR_TIMEOUT));  // clear TIMEOUT Interrupt
				ret = 0;
				break;
			}
		}
