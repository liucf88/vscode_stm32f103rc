...
uint8 parseDHCPMSG(uint16 length) {
  ...
  if (memcmp(pRIPMSG->chaddr, SRC_MAC_ADDR, 6) != 0 ||
    ...
    printf("\tSRC_MAC_ADDR(%02X.%02X.%02X.", SRC_MAC_ADDR[0], SRC_MAC_ADDR[1],
           SRC_MAC_ADDR[2]);
  ...
  while (p < e) {
...
#ifdef DHCP_DEBUG
      printf("subnetMask : ");
      printf("%u.%u.%u.%u\r\n", GET_SN_MASK[0], GET_SN_MASK[1], GET_SN_MASK[2],
             GET_SN_MASK[3]);
#endif
      break;
    ...
    case routersOnSubnet:
      ...
      printf("%u.%u.%u.%u\r\n", GET_GW_IP[0], GET_GW_IP[1], GET_GW_IP[2],
             GET_GW_IP[3]);
#endif
      break;
    ...
    case dhcpServerIdentifier:
      ...
      if (*((uint32 *)DHCP_SIP) == 0 ||
        ...
        printf("\tMY(%u.%u.%u.%u) ", DHCP_SIP[0], DHCP_SIP[1], DHCP_SIP[2],
               DHCP_SIP[3]);
  ...
  case STATE_DHCP_REREQUEST:
    if (type == DHCP_ACK) {
      if (memcmp(OLD_SIP, GET_SIP, 4) != 0) {
        ...
        printf("OLD_SIP=%u.%u.%u.%u", OLD_SIP[0], OLD_SIP[1], OLD_SIP[2],
               OLD_SIP[3]);
