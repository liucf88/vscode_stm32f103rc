...
void send_DHCP_DISCOVER(void)
{
	...
	memset((void *)pRIPMSG, 0,
		   sizeof(RIP_MSG)); /*清空pRIPMSG的 sizeof(RIP_MSG)	个空间*/
	...
	// set the host name
	sprintf((char *)host_name, "%.4s-%02X%02X%02X", DEVICE_ID, SRC_MAC_ADDR[3],
...
uint8 parseDHCPMSG(uint16 length)
{
...
#ifdef DHCP_DEBUG
	printf("DHCP_SIP:%u.%u.%u.%u\r\n", DHCP_SIP[0], DHCP_SIP[1], DHCP_SIP[2],
		   DHCP_SIP[3]);
	...
	if (memcmp(pRIPMSG->chaddr, SRC_MAC_ADDR, 6) != 0 ||
		...
		printf("\tSRC_MAC_ADDR(%02X.%02X.%02X.", SRC_MAC_ADDR[0],
			   SRC_MAC_ADDR[1], SRC_MAC_ADDR[2]);
		printf("%02X.%02X.%02X)", SRC_MAC_ADDR[3], SRC_MAC_ADDR[4],
			   SRC_MAC_ADDR[5]);
		printf(", pRIPMSG->chaddr(%02X.%02X.%02X.", (char)pRIPMSG->chaddr[0],
			   (char)pRIPMSG->chaddr[1], (char)pRIPMSG->chaddr[2]);
		printf("%02X.%02X.%02X)", pRIPMSG->chaddr[3], pRIPMSG->chaddr[4],
			   pRIPMSG->chaddr[5]);
		printf("\tpRIPMSG->xid(%08X), DHCP_XID(%08X)", pRIPMSG->xid,
			   (DHCP_XID));
		printf("\tpRIMPMSG->yiaddr:%d.%d.%d.%d\r\n", pRIPMSG->yiaddr[0],
			   pRIPMSG->yiaddr[1], pRIPMSG->yiaddr[2], pRIPMSG->yiaddr[3]);
#endif
		return 0;
	}
...
#ifdef DHCP_DEBUG
			printf(
				"Another DHCP sever send a response message. This is "
				"ignored.\r\n");
			printf("\tIP:%u.%u.%u.%u\r\n", svr_addr[0], svr_addr[1],
				   svr_addr[2], svr_addr[3]);
#endif
			return 0;
		}
	}
		...
		switch (*p++) {
			...
			case dhcpServerIdentifier:
				...
				if (*((uint32 *)DHCP_SIP) == 0 ||
					...
					*((uint32 *)DHCP_SIP) == *((uint32 *)svr_addr)) {
					memcpy(DHCP_SIP, p, 4);
					memcpy(DHCP_REAL_SIP, svr_addr,
						   4);  // Copy the real ip address of my DHCP server
#ifdef DHCP_DEBUG
					...
					printf("%u.%u.%u.%u\r\n", DHCP_REAL_SIP[0],
						   DHCP_REAL_SIP[1], DHCP_REAL_SIP[2],
						   DHCP_REAL_SIP[3]);
#endif
				} else {
#ifdef DHCP_DEBUG
					printf("Another dhcpServerIdentifier : \r\n");
					printf("\tMY(%u.%u.%u.%u) ", DHCP_SIP[0], DHCP_SIP[1],
						   DHCP_SIP[2], DHCP_SIP[3]);
...
uint8 check_DHCP_state(SOCKET s)
{
	...
	} else /*socket处于关闭状态，重新初始化socket*/
	{
		...
		if (!socket(SOCK_DHCP, Sn_MR_UDP, DHCP_CLIENT_PORT,
					0x00)) /*初始化socket和端口*/
		{
#ifdef DHCP_DEBUG
		...
		case STATE_DHCP_REREQUEST:
			if (type == DHCP_ACK) {
				if (memcmp(OLD_SIP, GET_SIP, 4) != 0) {
					...
					printf("OLD_SIP=%u.%u.%u.%u", OLD_SIP[0], OLD_SIP[1],
						   OLD_SIP[2], OLD_SIP[3]);
...
{
#ifdef DHCP_DEBUG
	printf("<Check the IP Conflict %d.%d.%d.%d: ", GET_SIP[0], GET_SIP[1],
		   GET_SIP[2], GET_SIP[3]);
#endif
...
void init_dhcp_client(void)
{
	...
	printf("mac=%02x:%02x:%02x:%02x:%02x:%02x\r\n", SRC_MAC_ADDR[0],
		   SRC_MAC_ADDR[1], SRC_MAC_ADDR[2], SRC_MAC_ADDR[3], SRC_MAC_ADDR[4],
		   SRC_MAC_ADDR[5]);
